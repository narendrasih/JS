...
x.rel = 'stylesheet';
x.type = 'textcss'
x.href = httpscdn.jsdelivr.netghEastArcticaJS-Plugins@masterUn-ReleasedKahootKahootModal.css
document.head.appendChild(x);

var xhttp = new XMLHttpRequest()
var URL = window.location.href
var found = URL.search(httpsquizizz.comjoingame);
...
if (found) {
	...
	if (!window.idiidk) {
		window.idiidk = function() {
			...
			fetch(httpsgame.quizizz.complay-apiv3getQuestions, {
				method POST,
				headers {
				...
				for (const o of Object.keys(t.questions)) {
					...
					const s = t.questions[o],
						...
						c = Encoding.decode(n);
						ModalContent.insertAdjacentHTML( 'beforeend', `bQuestion b`)
						...
						if (c[0] == [) {
							...
							ModalContent.insertAdjacentHTML( 'beforeend', `bAnswersbbr`)
						    ...
						    ModalContent.insertAdjacentHTML( 'beforeend', `bAnswerb ${s.structure.options[c].text  s.structure.options[c].media[0].url} brbr`)
						}
				}
			});
		}
